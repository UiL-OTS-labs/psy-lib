                         ...
                         GParamSpec *pspec)
{
    ...
    case PROP_NUM_CHANNELS:
        ...
        break;
    case PROP_IS_DECODED:
        g_value_set_boolean(value, priv->state == STATE_DECODED);
        break;
    case PROP_IS_UPLOADED:
        g_value_set_boolean(value, priv->state == STATE_UPLOADED);
        break;
    case PROP_WIDTH:
        g_value_set_uint(value, priv->image.width);
        break;
    case PROP_HEIGHT:
...
psy_texture_class_init(PsyTextureClass *klass)
{
     ...
     * from disc and decoded.
     */
    texture_properties[PROP_FILE] = g_param_spec_object(
        "file",
        "GIO.file",
    ...
    texture_properties[PROP_IS_DECODED]
        = g_param_spec_boolean("is-decoded",
                               "Is-Decoded",
     ...
     * is set to true.
     */
    texture_properties[PROP_IS_UPLOADED] = g_param_spec_boolean(
        "is-uploaded",
        "Is-Uploaded",
...
static void
decode_picture(PsyTexture *self, GByteArray *bytes, GFile *file, GError **error)
{
    int                width, height, num_channels;
    PsyTexturePrivate *priv = psy_texture_get_instance_private(self);

    guint8 *data = stbi_load_from_memory(bytes->data,
                                         (int) bytes->len,
                                         &width,
                                         &height,
                                         &num_channels,
             ...
             GCancellable *cancellable)
{
    ...
    tdata->img_description.image_data
...
on_error:
    ...
    if (error)
        ...
        g_task_return_pointer(
            task, tdata, (GDestroyNotify) texture_data_destroy);
}
                           ...
                           gpointer            data)
{
    ...
    g_task_set_task_data(task, tdata, (GDestroyNotify) texture_data_destroy);
