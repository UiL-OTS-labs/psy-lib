...
G_DEFINE_TYPE_WITH_PRIVATE(
        PsyRectangle, psy_rectangle, PSY_TYPE_VISUAL_STIMULUS
        )
...
psy_rectangle_class_init(PsyRectangleClass* klass)
{
     ...
     */
    rectangle_properties[PROP_WIDTH] = g_param_spec_float (
            "width",
            "Width",
     ...
     * three vertices are required. There is also one in the center of the rectangle
     * but that one is counted separately
     */
    rectangle_properties[PROP_HEIGHT] = g_param_spec_float(
            "height",
            "Height",
...
{
    return g_object_new(PSY_TYPE_RECTANGLE,
           "window", window,
           NULL);
}
